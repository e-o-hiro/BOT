...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ To fazendo lek, calma ae ⌛',
				success: 'Pronto ✔️',
				error: {
					stick: '❌ deu algum erro na criação do sticker, tenta dnv ❌',
					Iv: '❌ Link inválido ❌'
				},
				only: {
			...
			switch(command) {
				...
				case 'ocr':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Manda qualquer foto. Obs: vai vir animes aleatórios')
					}
					break
				case 'tp':
					if (args.length < 1) {
						return reply('Escolha o tema, lek, 1 - 162')
